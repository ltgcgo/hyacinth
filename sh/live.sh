...
esbuild --log-level=warning --log-limit=0 --bundle src/${1:-default}/index.${ext} $platform $prepend $append $inject $buildOpt --format=$format --charset=utf8 --preserve-symlinks --loader:.htm=text --loader:.css=text --loader:.svg=text --outfile=dist/${1:-default}.${ext} ${2:---minify-whitespace --minify-syntax --sourcemap --watch} $3
